...
class TypeOfBeneficiaryController @Inject()(
    ...
    formProvider: TypeOfBeneficiaryFormProvider,
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData).async {
    implicit request =>
      ...
      val json = Json.obj(
        "form"   -> preparedForm,
        "mode"   -> mode,
        "radios"  -> TypeOfBeneficiary.radios(preparedForm)
      )
      ...
      form.bindFromRequest().fold(
        formWithErrors => {
          ...
          val json = Json.obj(
            "form"   -> formWithErrors,
            "mode"   -> mode,
            "radios" -> TypeOfBeneficiary.radios(formWithErrors)
          )

          renderer.render("typeOfBeneficiary.njk", json).map(BadRequest(_))
        },
        value =>
          for {
