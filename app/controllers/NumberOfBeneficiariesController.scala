...
import models.{Mode, NumberOfBeneficiaries}
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      val json = Json.obj(
        "form"   -> preparedForm,
        "mode"   -> mode,
        "radios"  -> NumberOfBeneficiaries.radios(preparedForm)
      )

      renderer.render("numberOfBeneficiaries.njk", json).map(Ok(_))
  }
      ...
      form.bindFromRequest().fold(
        formWithErrors => {
          ...
          val json = Json.obj(
            "form"   -> formWithErrors,
            "mode"   -> mode,
            "radios" -> NumberOfBeneficiaries.radios(formWithErrors)
          )

          renderer.render("numberOfBeneficiaries.njk", json).map(BadRequest(_))
        },
        value =>
          for {
